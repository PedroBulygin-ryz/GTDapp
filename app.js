...
function renderShortcutHelp() {
  return `
    <aside class="shortcut-help">
      ...
      <p><strong>Control + Option + B</strong>: con <strong>Bubbles Capture</strong> abierta, muestra una ventanita para escribir una captura y guardarla en <code>inbox.txt</code>.</p>
...
function renderReviewView() {
  ...
  const dueSummary = getReviewDueSummary();
  ...
  els.focus.innerHTML = `
    ...
    <section class="review-grid">
      ...
      ${renderDueReviewCard(dueSummary)}
    ...
    <section class="stage-list">
      <button class="section-title section-title-button" data-toggle-review-history>
        <h3>Historial reciente</h3>
        <div class="section-title-meta">
          <small>${state.reviewHistoryOpen ? "Ocultar" : "Abrir"}</small>
          <span>${recentHistory.length}</span>
        </div>
      </button>
      ${
        state.reviewHistoryOpen
          ? `${renderRecentHistory(visibleHistory)}${renderHistoryPagination(recentHistory.length, historyPageCount)}`
          : ""
      }
    </section>
  `;
...
function renderDueReviewCard(summary) {
  const needsWork = summary.overdue > 0 || summary.today > 0;
  return `
    <article class="review-card due-review-card ${needsWork ? "needs-work" : "ok"}">
      <span>${summary.next7}</span>
      <h3>Vencimientos</h3>
      <p>No incluye tareas cerradas.</p>
      <dl class="due-summary">
        <div>
          <dt>Vencidas</dt>
...
function getReviewDueSummary() {
  const today = startOfDay(new Date());
  const tomorrow = addDays(today, 1);
  const weekLimit = addDays(today, 7);
  return state.tasks
    .filter((task) => task.bubble !== "done" && task.dueDate)
    .reduce(
      (summary, task) => {
        const due = parseDueDate(task.dueDate);
        if (!due) return summary;
        if (due < today) summary.overdue += 1;
        if (toDateKey(due) === toDateKey(today)) summary.today += 1;
        if (toDateKey(due) === toDateKey(tomorrow)) summary.tomorrow += 1;
        if (due >= today && due <= weekLimit) summary.next7 += 1;
        return summary;
      },
      { overdue: 0, today: 0, tomorrow: 0, next7: 0 },
    );
}
...
document.addEventListener("click", (event) => {
  ...
  if (event.target.closest("[data-toggle-review-history]")) {
    state.reviewHistoryOpen = !state.reviewHistoryOpen;
    state.historyPage = 0;
    saveAndRender();
    return;
  }
